...
This update is focused on underlying API-changes. There is no user-facing changes in interface.

## Deprecations

* Kraken no longer supports open-interest. The underlying new data-structure is not properly documented, and given the time-constraints it has been deprecated. If there is any demand for it, it will be reintroduced.

* The charting aspect of the package have been experimental for a long time. Ever since its inception actually. There is currently a new R package under development: {talib}. {cryptoQuotes} will in later versions take dependency on this package and use the function there for the charting interface.
